...
async def cmd_send_start(user_id: int, state: FSMContext, session: AsyncSession, *, app_context: AppContext):
...
async def cmd_send_callback(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_send_message(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_send_token(message: types.Message, state: FSMContext, session: AsyncSession,
                         send_for: str, send_asset: Asset, send_sum: float, send_memo: Union[str, None] = None, *, app_context: AppContext):
    ...
    try:
        if '@' == send_for[0]:
            ...
            if tmp_name is TELEGRAM_API_ERROR:
                ...
                await send_message(session, user_id_to_notify, my_gettext(user_id_to_notify, 'telegram_api_error', app_context=app_context),
            ...
            actual_username_lower: str | None = tmp_name.lower() if isinstance(tmp_name, str) and tmp_name else None
        ...
        await send_message(session, user_id_to_notify, my_gettext(user_id_to_notify, 'send_error2', app_context=app_context),
                           reply_markup=get_kb_return(message, app_context=app_context), app_context=app_context)
        return
...
async def cmd_eurmtl(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    await clear_state(state)
    ...
    try:
        ...
        await send_message(session, chat_id, 'Invalid amount. Please enter a numerical value.', app_context=app_context)
        return
...
async def cmd_start_eurmtl(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    await message.delete()
    await clear_state(state)
    ...
    if not await check_user_id(session, user_id):
        await send_message(session, user_id, 'You dont have wallet. Please run /start', app_context=app_context)
        return

    # Parse the start command
    parts = message.text.split('eurmtl_')[1].strip().split('-') if message.text else []
    if len(parts) < 2:
        await send_message(session, user_id, 'Invalid command format', app_context=app_context)
        return
...
async def cmd_send_for(message: Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    if message.from_user is None:
        return
    ...
    if my_account:
        ...
        if address and isinstance(address, str) and (not free_wallet) and (len(address) == 56) and (address[0] == 'G'):  # need activate
...
async def cmd_send_choose_token(message: types.Message, state: FSMContext, session: AsyncSession, *, app_context: AppContext):
    if message.from_user is None:
        return
    data = await state.get_data()
    ...
    for token in asset_list:
        ...
        for sender_token in sender_asset_list:
            if token.asset_code == sender_token.asset_code and token.asset_issuer == sender_token.asset_issuer:
...
async def cq_send_cancel_offers_click(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
    """
...
async def cmd_send_get_sum(message: Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    if message.from_user is None:
        return
    try:
...
async def cmd_send_04(session: AsyncSession, message: types.Message, state: FSMContext, need_new_msg=None, *, app_context: AppContext):
    data = await state.get_data()
    ...
    msg = my_gettext(
        message,
        'confirm_send',
        (float2str(send_sum), send_asset_name, send_address + ' ' + mtlap_stars, send_memo_str),
        app_context=app_context
    )
    if cancel_offers:
    ...
    await state.update_data(xdr=xdr, operation='send', msg=None,
                            success_msg=my_gettext(message, 'confirm_send_success',
                                                   (float2str(send_sum), send_asset_name,
                                                    send_address + ' ' + mtlap_stars, send_memo_str), app_context=app_context))
...
async def cmd_get_memo(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_send_memo(message: Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_create_account(user_id: int, state: FSMContext, session: AsyncSession, *, app_context: AppContext):
    data = await state.get_data()
...
async def cmd_send_15_xlm(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def handle_docs_photo(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    if message.from_user is None:
        return
