...
async def get_kb_default(
    session: AsyncSession, chat_id: int, state: FSMContext, *, app_context: AppContext
        ...
        buttons = [
            ...
            [
                types.InlineKeyboardButton(
                    text="⤴️ Send USDt", callback_data="SendTonUSDt"
                )
            ],
            [
                types.InlineKeyboardButton(
                    text="↔️ "
                    + my_gettext(chat_id, "kb_change_wallet", app_context=app_context),
            ...
            [
                types.InlineKeyboardButton(
                    text="ℹ️ "
                    + my_gettext(chat_id, "kb_support", app_context=app_context),
    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text="⤵️ " + my_gettext(chat_id, "kb_receive", app_context=app_context),
                callback_data="Receive",
            ),
            types.InlineKeyboardButton(
                text="🔃 " + my_gettext(chat_id, "kb_refresh", app_context=app_context),
                callback_data="Refresh",
            ),
            types.InlineKeyboardButton(
                text="⤴️ " + my_gettext(chat_id, "kb_send", app_context=app_context),
                callback_data="Send",
            ),
        ],
        [
            types.InlineKeyboardButton(
                text="🔄 " + my_gettext(chat_id, "kb_swap", app_context=app_context),
                callback_data="Swap",
            ),
            types.InlineKeyboardButton(
                text="💸 " + my_gettext(chat_id, "kb_inout", app_context=app_context),
                callback_data="InOut",
            ),
            types.InlineKeyboardButton(
                text="📊 " + my_gettext(chat_id, "kb_market", app_context=app_context),
    ...
    if data.get("show_more", False):
            ...
            buttons.append(
                [
                    types.InlineKeyboardButton(
                        text="🖇 "
                        + my_gettext(
                            chat_id, "kb_mtlap_tools", app_context=app_context
                        ),
        ...
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text="🏛 "
                    + my_gettext(chat_id, "kb_mtl_tools", app_context=app_context),
                    callback_data="MTLTools",
                ),
                types.InlineKeyboardButton(
                    text="⚙️ "
                    + my_gettext(chat_id, "kb_setting", app_context=app_context),
        ...
        if not is_free:
            buttons.append(
                [
                    types.InlineKeyboardButton(
                        text="🖌 "
                        + my_gettext(chat_id, "kb_sign", app_context=app_context),
                        callback_data="Sign",
                    )
                ]
            )
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text="≢ "
                    + my_gettext(chat_id, "kb_show_less", app_context=app_context),
        ...
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text="≡ "
                    + my_gettext(chat_id, "kb_show_more", app_context=app_context),
                    callback_data="ShowMoreToggle",
                )
            ]
        )
        ...
        try:
            ...
            if refresh_callback and msg == data.get("start_msg"):
                await refresh_callback.answer(
                    "Nothing to update, the data is up to date.", show_alert=True
                )
                await state.update_data(start_msg=msg)
            else:
                keyboard = await get_kb_default(
                    session, user_id, state, app_context=app_context
                )
                await send_message(
                    session,
                    user_id,
            ...
            kb = [
                [
                    types.InlineKeyboardButton(
                        text=my_gettext(
                            user_id, "kb_change_wallet", app_context=app_context
                        ),
                        callback_data="ChangeWallet",
                    )
                ]
            ...
            await send_message(
                session,
                user_id,
                my_gettext(user_id, "load_error", app_context=app_context),
...
async def get_start_text(
    ...
    if await secret_service.is_ton_wallet(user_id):
        ...
        address_str = (
            ton_service.wallet.address.to_str(is_bounceable=False)
            if ton_service.wallet
            else "Unknown"
        )
        return f"""Address: 
...
{warning_message}"""
    ...
    msg = (
        f'<a href="{link}">{simple_account}</a> {info} {my_gettext(user_id, "your_balance", app_context=app_context)}\n\n'
        f"{balance_str}"
    )
    return msg


async def cmd_info_message(
    session: Optional[AsyncSession] = None,
    user_id: Union[types.CallbackQuery, types.Message, int] = 0,
    msg: str = "",
    send_file=None,
    resend_transaction=None,
    ...
    *,
    ...
    if app_context:
        ...
        if not current_dp:
            current_dp = app_context.dispatcher
        if not loc_service:
            loc_service = app_context.localization_service

    # Create minimal context if not provided (for notification service)
    if not app_context and current_bot and current_dp:
        from unittest.mock import MagicMock

        app_context = MagicMock()
        app_context.bot = current_bot
        app_context.dispatcher = current_dp
        app_context.localization_service = loc_service
    ...
    if send_file:
        ...
        if current_dp and current_dp.storage:
            ...
            with suppress(TelegramBadRequest):
                await current_bot.delete_message(
                    user_id, data.get("last_message_id", 0)
                )
        ...
        await send_message(
            ...
            user_id,
            msg,
            reply_markup=get_kb_resend(user_id, app_context=app_context),
            bot=current_bot,
            app_context=app_context,
        )
    elif operation_id:
...
async def cmd_change_wallet(
    ...
    for wallet in wallets:
        ...
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text=f"{wallet.public_key[:4]}..{wallet.public_key[-4:]}",
                    callback_data=WalletSettingCallbackData(
                        action="NAME", idx=wallet.id
                    ).pack(),
                ),
                types.InlineKeyboardButton(
                    text=f"{active_name}",
                    callback_data=WalletSettingCallbackData(
                        action="SET_ACTIVE", idx=wallet.id
                    ).pack(),
                ),
                types.InlineKeyboardButton(
                    text="Delete",
                    callback_data=WalletSettingCallbackData(
                        action="DELETE", idx=wallet.id
                    ).pack(),
                ),
            ]
        )
    buttons.append(
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_add_new", app_context=app_context),
    ...
    await state.update_data(
        wallets={wallet.id: wallet.public_key for wallet in wallets}
    )
