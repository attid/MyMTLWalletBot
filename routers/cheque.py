...
async def cmd_create_cheque(
    ...
    await send_message(
        ...
        update,
        msg,
        reply_markup=get_kb_return(update, app_context=app_context),
...
async def cmd_cheque_show(
    session: AsyncSession, message: Message, state: FSMContext, app_context: AppContext
):
    ...
    await send_message(
        ...
        message,
        msg,
        reply_markup=get_kb_send_cheque(message.from_user.id, app_context),
        app_context=app_context,
    )


def get_kb_send_cheque(
    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_create_cheque", app_context=app_context),
                callback_data="ChequeExecute",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_change_amount", app_context=app_context),
                callback_data="CreateCheque",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_change_count", app_context=app_context),
                callback_data="ChequeCount",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_change_comment", app_context=app_context),
                callback_data="ChequeComment",
            )
        ],
...
async def cmd_cheque_get_count(
    message: Message, state: FSMContext, session: AsyncSession, app_context: AppContext
):
...
async def cmd_cheque_get_comment(
    message: Message, state: FSMContext, session: AsyncSession, app_context: AppContext
):
    if message.text is None:
        return
...
async def cheque_after_send(
    ...
    **kwargs,
    ...
    if cheque is None:
        ...
        cheque = await repo.create(
            send_uuid, str(send_sum), send_count, user_id, send_comment
        )
    ...
    kb = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=my_gettext(user_id, "kb_send_cheque", app_context=app_context),
                    switch_inline_query="",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text=my_gettext(user_id, "kb_cheque_info", app_context=app_context),
            ...
            [
                types.InlineKeyboardButton(
                    text=my_gettext(
                        user_id, "kb_cancel_cheque", app_context=app_context
                    ),
                    callback_data=ChequeCallbackData(
                        uuid=send_uuid, cmd="cancel"
                    ).pack(),
                )
            ],
            get_return_button(user_id, app_context=app_context),
...
async def cb_cheque_click(
    ...
    if cmd == "info":
        await callback.answer(
            f"Cheque was received {receive_count} from {total_count}", show_alert=True
        )
    elif cmd == "cancel":
        if total_count > receive_count:
            app_context.cheque_queue.put_nowait(
                ChequeQuery(
...
async def cmd_cancel_cheque(
    ...
    if not result.success:
        await cmd_info_message(
            session, user_id, f"Error: {result.error_message}", app_context=app_context
        )
        return
    ...
    await cmd_info_message(
        session,
        user_id,
        my_gettext(user_id, "send_good_cheque", app_context=app_context),
        app_context=app_context,
    )
...
async def cmd_inline_query(
    ...
    for record in data:
        if record.status == ChequeStatus.CHEQUE.value:
            ...
            button_text = my_gettext(
                inline_query.from_user.id, "kb_get_cheque", app_context=app_context
            )
            ...
            msg = my_gettext(
                inline_query.from_user.id,
                "inline_invoice_buy",
                (record.amount, asset_code, record.comment),
                app_context=app_context,
            )
            button_text = my_gettext(
                inline_query.from_user.id, "kb_get_invoice_buy", app_context=app_context
            )
        ...
        results.append(
            types.InlineQueryResultArticle(
                ...
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [types.InlineKeyboardButton(text=button_text, url=link)]
                    ]
                ),
            )
        )
    ...
    await inline_query.answer(
        results[:49],
        is_personal=True,  # type: ignore[arg-type]
    )
...
async def cmd_start_cheque(
    ...
    if not cheque or user_receive_count > 0 or receive_count >= cheque.count:
        await send_message(
            session,
            user_id,
            my_gettext(user_id, "bad_cheque", app_context=app_context),
    ...
    if cheque.status == ChequeStatus.CHEQUE.value:
        msg = my_gettext(
            user_id,
            "inline_cheque",
            (cheque.amount, cheque.count, cheque.comment),
            app_context=app_context,
        )
        kb = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=my_gettext(
                            user_id, "kb_get_cheque", app_context=app_context
                        ),
                        callback_data="ChequeYes",
                    )
                ],
                get_return_button(user_id, app_context=app_context),
            ]
        ...
        msg = my_gettext(
            user_id,
            "inline_invoice_buy",
            (cheque.amount, asset_code, cheque.comment),
            app_context=app_context,
        )
        kb = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=my_gettext(
                            user_id, "kb_get_invoice_buy", app_context=app_context
                        ),
                        callback_data="InvoiceYes",
                    )
                ],
                get_return_button(user_id, app_context=app_context),
            ]
...
async def cmd_send_money_from_cheque(
    session: AsyncSession,
    user_id: int,
    state: FSMContext,
    cheque_uuid: str,
    ...
    result = await use_case.execute(
        user_id=user_id, cheque_uuid=cheque_uuid, username=username
    )
...
async def cheque_worker(app_context: AppContext):
    while True:  # not queue.empty():
        ...
        try:
            ...
            logger.warning(
                f" {cheque_item.cheque_uuid}-{cheque_item.user_id} failed {type(e)}"
            )
        app_context.cheque_queue.task_done()


@router.callback_query(F.data == "InvoiceYes")
async def cmd_invoice_yes(
    callback: CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    await state.update_data(
        send_asset_code=eurmtl_asset.code,
