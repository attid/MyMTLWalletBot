...
async def cmd_add_new(callback: types.CallbackQuery, session: AsyncSession, app_context: AppContext):
    buttons = [
...
async def cq_add_have_key(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_sending_private(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    ...
    try:
        ...
        assert app_context.encryption_service is not None, "encryption_service must be initialized"
...
async def cq_add_new_key(callback: types.CallbackQuery, session: AsyncSession, state: FSMContext, app_context: AppContext):
    ...
    try:
        ...
        async with new_wallet_lock:
             ...
             assert master_wallet.secret_key is not None, "master_wallet.secret_key must not be None"
...
async def cmd_show_add_wallet_choose_pin(session: AsyncSession, user_id: int, state: FSMContext, msg='', *, app_context: AppContext):
    buttons = [
...
async def cq_add_read_only(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cmd_sending_public(message: types.Message, state: FSMContext, session: AsyncSession, app_context: AppContext):
    if message.from_user is None or message.text is None:
        return
    try:
...
async def cq_add_read_only_pin(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
    assert callback.message is not None and callback.message.chat is not None, "callback.message and chat must not be None"
...
async def cq_add_password(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cq_add_read_only_no_password(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, app_context: AppContext):
...
async def cq_add_ton(callback: types.CallbackQuery, session: AsyncSession, state: FSMContext, app_context: AppContext):
    if callback.from_user is None or callback.message is None or callback.message.chat is None:
        return
    try:
        assert app_context.use_case_factory is not None, "use_case_factory must be initialized"
