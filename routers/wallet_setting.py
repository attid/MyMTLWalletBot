...
async def cmd_wallet_setting(
    ...
    if free_wallet:
        private_button = types.InlineKeyboardButton(
            text=my_gettext(callback, "kb_buy", app_context=app_context),
            callback_data="BuyAddress",
        )
    else:
        private_button = types.InlineKeyboardButton(
            text=my_gettext(callback, "kb_get_key", app_context=app_context),
    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_manage_assets", app_context=app_context),
                callback_data="ManageAssetsMenu",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_address_book", app_context=app_context),
                callback_data="AddressBook",
            )
        ],
        ...
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_set_password", app_context=app_context),
        ...
        [
            types.InlineKeyboardButton(
                text=my_gettext(
                    callback, "kb_remove_password", app_context=app_context
                ),
                callback_data="RemovePassword",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_donate", app_context=app_context),
                callback_data="Donate",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_set_default", app_context=app_context),
                callback_data="SetDefault",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_set_limit", app_context=app_context),
                callback_data="SetLimit",
            )
        ],
        [
            types.InlineKeyboardButton(
                text="🔕 "
                + my_gettext(
                    callback, "kb_notification_settings", app_context=app_context
                ),
        ...
        [
            types.InlineKeyboardButton(
                text="🌐 "
                + my_gettext(callback, "change_lang", app_context=app_context),
...
async def cmd_manage_assets(
    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_delete_one", app_context=app_context),
                callback_data="DeleteAsset",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_add_list", app_context=app_context),
                callback_data="AddAsset",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(callback, "kb_add_expert", app_context=app_context),
                callback_data="AddAssetExpert",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(
                    callback, "kb_asset_visibility", app_context=app_context
                ),
                callback_data="AssetVisibilityMenu",
            )
        ],
        get_return_button(callback, app_context=app_context),
    ]
...
async def _generate_asset_visibility_markup(
    user_id: int, session: AsyncSession, app_context: AppContext, page: int = 1
    ...
    for asset in assets_on_page:
        ...
        exchange_only_text = my_gettext(
            user_id, "asset_exchange_only", app_context=app_context
        )
        ...
        exchange_only_button = types.InlineKeyboardButton(
            ...
            callback_data=AssetVisibilityCallbackData(
                action="set", code=code, status=1, page=page
            ).pack(),
        )
        hidden_button = types.InlineKeyboardButton(
            ...
            callback_data=AssetVisibilityCallbackData(
                action="set", code=code, status=2, page=page
            ).pack(),
        )
    ...
    )  # Assuming get_return_button returns a list containing the button
...
async def cmd_asset_visibility_menu(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    message_text, reply_markup = await _generate_asset_visibility_markup(
        user_id, session, app_context, page=1
    )
...
async def handle_asset_visibility_action(
    callback: types.CallbackQuery,
    callback_data: AssetVisibilityCallbackData,
    state: FSMContext,
    session: AsyncSession,
    ...
    logger.info(
        f"Entered handle_asset_visibility_action with callback_data: {callback_data!r}"
    )  # Log entry point
    ...
    if action == "page":
        ...
        target_page = (
            callback_data.page
        )  # The page number is directly in callback_data for 'page' action
        message_text, reply_markup = await _generate_asset_visibility_markup(
            user_id, session, app_context, page=target_page
        )
        try:
            ...
            await callback.answer(
                my_gettext(callback, "error_refreshing_menu", app_context=app_context),
                show_alert=True,
        ...
        )  # The integer status associated with the button pressed (1 or 2)
        ...
        if target_status_str is None:
            logger.error(
                f"Invalid target status integer received: {target_status_int} for asset {asset_code}"
            )
            await callback.answer(
                my_gettext(
                    callback, "error_processing_request", app_context=app_context
                ),
                show_alert=True,
            )
        ...
        if wallet is None:
            ...
            return
        vis_dict = (
            deserialize_visibility(wallet.assets_visibility)
            if wallet.assets_visibility
            else {}
        )
        ...
        if current_status_str == target_status_str:
            ...
            new_status_str = (
                target_status_str  # Clicking unchecked button -> set to target
            )
        ...
        if new_status_str == ASSET_VISIBLE:
            vis_dict.pop(
                asset_code, None
            )  # Remove from dict if setting back to default visible
        else:
        ...
        try:
            ...
            await callback.answer(
                my_gettext(callback, "error_saving_settings", app_context=app_context),
                show_alert=True,
            )
            save_error = True
        ...
        if not save_error:
            ...
            message_text, reply_markup = await _generate_asset_visibility_markup(
                user_id, session, app_context, page=page
            )
            try:
                if callback.message and isinstance(callback.message, types.Message):
                    await callback.message.edit_text(
                ...
                await callback.answer(
                    my_gettext(
                        callback, "asset_visibility_changed", app_context=app_context
                    )
                )
                ...
                if "message is not modified" in str(e):
                    ...
                    logger.error(
                        f"TelegramBadRequest editing message after asset visibility change: {e}"
                    )
                    await callback.answer(
                        my_gettext(
                            callback,
                ...
                logger.error(
                    f"Error editing message after asset visibility change: {e}"
                )
        ...
        logger.warning(
            f"Received legacy 'toggle' action for asset visibility from user {user_id}"
        )
        await callback.answer(
            "Legacy action received.", show_alert=True
...
async def cq_swap_choose_token_from(
    callback: types.CallbackQuery,
    ...
    if asset:
        ...
        await state.update_data(
            send_asset_code=asset_obj.asset_code,
        ...
        has_offers = any(
            o["selling"]["asset_code"] == asset_obj.asset_code
            and o["selling"]["asset_issuer"] == asset_obj.asset_issuer
            for o in offers
        )
        ...
        if has_offers:
            await send_message(
                session,
                callback,
                my_gettext(callback, "close_asset_has_offers", app_context=app_context),
        ...
        if xdr:
            ...
            msg = my_gettext(
                callback,
                "confirm_close_asset",
                (asset_obj.asset_code, asset_obj.asset_issuer),
                app_context=app_context,
            )
            await send_message(
                session,
        ...
        await callback.answer(
            my_gettext(callback, "bad_data", app_context=app_context), show_alert=True
        )
...
async def cmd_add_asset_add(
    ...
    if is_free and (len(await balance_use_case.execute(user_id=user_id)) > 5):
        await send_message(
            session,
            user_id,
            my_gettext(user_id, "only_3", app_context=app_context),
    ...
    if not xlm or float(xlm.balance) <= 0.5:
        await callback.answer(
            my_gettext(callback, "low_xlm", app_context=app_context), show_alert=True
        )
        return
    ...
    if len(good_asset) == 0:
        await send_message(
            session,
            user_id,
            my_gettext(user_id, "have_all", app_context=app_context),
            reply_markup=get_kb_return(user_id, app_context=app_context),
            app_context=app_context,
        )
        return False
    ...
    for key in good_asset:
        kb_tmp.append(
            [
                types.InlineKeyboardButton(
                    text=f"{key.asset_code}",
                    callback_data=AddAssetCallbackData(answer=key.asset_code).pack(),
                )
            ]
        )
    kb_tmp.append(get_return_button(callback, app_context=app_context))
    await send_message(
        session,
        user_id,
        my_gettext(user_id, "open_asset", app_context=app_context),
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=kb_tmp),
        app_context=app_context,
    )
...
async def cq_add_asset(
    ...
    if asset:
        await state.update_data(
            send_asset_code=asset[0].asset_code, send_asset_issuer=asset[0].asset_issuer
        )
        if callback.message is None or not isinstance(callback.message, types.Message):
            return
        await cmd_add_asset_end(
...
async def cmd_add_asset_expert(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_start_cheque(
    ...
    await send_message(
        session, message.from_user.id, "Loading", app_context=app_context
    )
...
async def cmd_add_asset_end(
    chat_id: int, state: FSMContext, session: AsyncSession, *, app_context: AppContext
):
    data = await state.get_data()
    ...
    await send_message(
        ...
        chat_id,
        msg,
        reply_markup=get_kb_yesno_send_xdr(chat_id, app_context=app_context),
        app_context=app_context,
    )
...
async def remove_password(
    ...
    await change_pw_use_case.execute(
        user_id=user_id, old_pin=pin, new_pin=str(user_id), pin_type=0
    )
    await session.commit()
    await state.set_state(None)
    await cmd_info_message(
        session, user_id, "Password was unset", app_context=app_context
    )


@router.callback_query(F.data == "RemovePassword")
async def cmd_remove_password(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_set_password(
    ...
    if pin_type in (1, 2):
        ...
        if is_free:
            await callback.answer(
                "You have free account. Please buy it first.", show_alert=True
            )
        else:
            if wallet is None:
                return
            ...
            await cmd_show_add_wallet_choose_pin(
                ...
                state,
                my_gettext(
                    callback, "for_address", (public_key,), app_context=app_context
                ),
                app_context=app_context,
            )
            await callback.answer()
...
async def send_private_key(
    ...
    if not secrets:
        await send_message(
            ...
            user_id,
            "Error: Incorrect PIN",
            reply_markup=get_kb_del_return(user_id, app_context=app_context),
            app_context=app_context,
        )
        return
...
async def cmd_buy_private_key(
    ...
    if is_free:
        ...
        await state.update_data(
            buy_address=public_key, fsm_after_send=jsonpickle.dumps(cmd_after_buy)
        )
        # Refactored to use UseCaseFactory
        ...
        if eurmtl_balance < config.wallet_cost:
            ...
            msg = my_gettext(
                callback,
                "confirm_send",
                (config.wallet_cost, eurmtl_asset.code, father_key, memo),
                app_context=app_context,
            )
        ...
        await callback.answer(
            "You can`t buy. You have you oun account. But you can donate /donate",
            show_alert=True,
        )


async def cmd_edit_address_book(
    ...
    await send_message(
        session,
        user_id,
        my_gettext(user_id, "address_book", app_context=app_context),
...
async def cb_edit_address_book(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cq_setting_address_book(
    callback: types.CallbackQuery,
    callback_data: AddressBookCallbackData,
    state: FSMContext,
    session: AsyncSession,
...
async def cmd_data_management(
        ...
        [
            types.InlineKeyboardButton(
                ...
                web_app=types.WebAppInfo(
                    url=f"https://eurmtl.me/ManageData"
    ...
    await send_message(
        session,
        callback,
        "There you can manage your data:",
...
async def cq_manage_data_callback(
    callback: types.CallbackQuery,
    callback_data: MDCallbackData,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
):
    ...
    json_payload = {"uuid": uuid_callback, "user_id": callback.from_user.id}
    status, json_data = await get_web_request(
        "POST",
        url="https://eurmtl.me/remote/get_mmwb_transaction",
        headers=headers,
        json=json_payload,
        return_type="json",
    )
