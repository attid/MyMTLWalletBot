    ...
    async def execute(
        ...
        if amount <= 0 or math.isinf(amount):
            return PaymentResult(
                success=False,
                error_message="Amount must be positive and finite (not unlimited)",
            )
        ...
        if create_account:
            if exists:
                ...
                return PaymentResult(
                    success=False, error_message="Destination account already exists"
                )
            if asset.code != "XLM":
                return PaymentResult(
                    success=False,
                    error_message="Can only create account with XLM (native asset)",
                )
        else:
            if not exists:
                return PaymentResult(
                    success=False, error_message="Destination account does not exist"
                )
