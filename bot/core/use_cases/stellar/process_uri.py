...
class ProcessStellarUri:
    def __init__(
        self, wallet_repo: IWalletRepository, stellar_service: IStellarService
    ):
            ...
            if uri_object.replace:
                ...
                if not wallet:
                    return ProcessStellarUriResult(
                        success=False, error_message="User wallet not found"
                    )

                # Fetch generic account details (to get sequence number)
                account_details = await self.stellar_service.get_account_details(
                    wallet.public_key
                )
                if not account_details:
                    return ProcessStellarUriResult(
                        success=False, error_message="Account details not found"
                    )

                from stellar_sdk import Account

                source_account = Account(
                    wallet.public_key, int(account_details["sequence"])
                )
    ...
    def _parse_transaction_stellar_uri(
        self, uri_data, network_passphrase=Network.PUBLIC_NETWORK_PASSPHRASE
    ):
        uri_object = stellar_uri.TransactionStellarUri.from_uri(
            uri_data, network_passphrase=network_passphrase
        )
        ...
        if hasattr(uri_object, "return_url"):
            ...
            hasattr(uri_object, "operation_attrs")
            and "return_url" in uri_object.operation_attrs
        ):
