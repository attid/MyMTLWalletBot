...
class ClaimCheque:
    ...
    async def execute(
        self, user_id: int, cheque_uuid: str, username: str
    ) -> ClaimResult:
        ...
        user_receive_count = await self.cheque_repository.get_receive_count(
            cheque_uuid, user_id
        )
        ...
        if not master_wallet.secret_key:
            return ClaimResult(
                False, error_message="Master wallet secret not available"
            )
        ...
        if was_new:
            ops.append(
                self.stellar_service.create_create_account_op(
            ...
            for asset_code in assets_to_trust:
                ops.append(
                    self.stellar_service.create_change_trust_op(
        ...
        ops.append(
            self.stellar_service.create_payment_op(
                destination=wallet.public_key,
                asset_code=cheque_asset_parts[0],
                asset_issuer=cheque_asset_parts[1]
                if len(cheque_asset_parts) > 1
                else None,
        ...
        tx = await self.stellar_service.build_transaction(
            source_public_key=source_pk, operations=ops, memo=cheque_uuid[:16]
        )
        ...
        if was_new or source_pk == master_wallet.public_key:
            tx_envelope = await self.stellar_service.sign_transaction(
                tx_envelope, master_secret
            )
        ...
        if was_new:
            ...
            user_secret = self.encryption_service.decrypt(
                wallet.secret_key, str(user_id)
            )
            if not user_secret:
                return ClaimResult(
                    False, error_message="Failed to decrypt wallet secret"
                )

            tx_envelope = await self.stellar_service.sign_transaction(
                tx_envelope, user_secret
            )
