...
class SqlAlchemyChequeRepository(IChequeRepository):
    ...
    async def get_by_uuid(
        ...
        stmt = select(MyMtlWalletBotCheque).where(
            MyMtlWalletBotCheque.cheque_uuid == uuid
        )
    ...
    async def get_available(self, user_id: int) -> List[Cheque]:
        ...
        subq = (
            select(MyMtlWalletBotCheque.cheque_id)
            .outerjoin(
                MyMtlWalletBotChequeHistory,
                MyMtlWalletBotCheque.cheque_id == MyMtlWalletBotChequeHistory.cheque_id,
            )
            .where(MyMtlWalletBotCheque.user_id == user_id)
            .where(MyMtlWalletBotCheque.cheque_status != ChequeStatus.CANCELED.value)
            .group_by(MyMtlWalletBotCheque.cheque_id, MyMtlWalletBotCheque.cheque_count)
            .having(
                func.count(MyMtlWalletBotChequeHistory.cheque_id)
    ...
    async def add_history(self, cheque_id: int, user_id: int) -> None:
        ...
        new_history = MyMtlWalletBotChequeHistory(
            user_id=user_id, dt_block=datetime.now(), cheque_id=cheque_id
        )
    ...
    async def cancel(self, cheque_uuid: str, user_id: int) -> bool:
        ...
        stmt = (
            select(MyMtlWalletBotCheque)
            .where(MyMtlWalletBotCheque.cheque_uuid == cheque_uuid)
            .where(MyMtlWalletBotCheque.user_id == user_id)
        )
        result = await self.session.execute(stmt)
        db_cheque = result.scalar_one_or_none()
