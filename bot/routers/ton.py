...
async def cmd_send_ton_start(
    ...
    await send_message(
        ...
        reply_markup=get_kb_return(callback.from_user.id, app_context=app_context),
...
async def cmd_send_ton_address(
    ...
    if len(address) != 48:
        await send_message(
            ...
            "Invalid address. Please enter a valid TON address.",
...
async def cmd_send_ton_sum(
    ...
    await send_message(
        ...
        reply_markup=get_kb_ton_yesno(message.from_user.id),
        app_context=app_context,
    )
...
async def cmd_send_ton_confirm(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    from infrastructure.services.wallet_secret_service import (
        SqlAlchemyWalletSecretService,
    )

    if app_context.use_case_factory:
        secret_service = app_context.use_case_factory.create_wallet_secret_service(
            session
        )
    else:
    ...
    if await secret_service.is_ton_wallet(user_id):
        try:
            ...
            await send_message(
                session, user_id, "Sending transaction...", app_context=app_context
            )
            ...
            if result:
                await send_message(
                    session,
                    user_id,
                    f"Successfully sent {amount} TON to \n<code>{recipient_address}</code>",
            ...
            await send_message(
                session, user_id, "An error occurred: ", app_context=app_context
            )
        ...
        await send_message(
            session,
            user_id,
            "Your default wallet is not a TON wallet.",
            app_context=app_context,
        )
...
async def cmd_send_ton_usdt_start(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_send_ton_usdt_address(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_send_ton_usdt_sum(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_send_ton_usdt_confirm(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
        ...
        try:
            ...
            if result:
                await send_message(
                    session,
                    user_id,
                    f"Successfully sent {amount} USDT to \n<code>{recipient_address}</code>",
                    reply_markup=get_kb_return(
                        callback.from_user.id, app_context=app_context
                    ),
                    app_context=app_context,
...
async def cmd_send_ton_usdt_cancel(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
