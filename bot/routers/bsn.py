...
def make_tag_message(
    bsn_data: BSNData,
    user_id: typing.Union[CallbackQuery, Message, int, str],
    app_context: AppContext,
) -> str:
    user_id_int: int = (
        int(user_id) if isinstance(user_id, str) else get_user_id(user_id)
    )
    help_text = my_gettext(
        user_id_int, "bsn_help_text", (DELETE_KEY,), app_context=app_context
    )
...
def get_bsn_kb(
    ...
    if send_enabled:
        builder.button(
            text=my_gettext(user_id_int, "kb_send", app_context=app_context),
            callback_data=SEND_CALLBACK_DATA,
        )
    builder.button(
        text=my_gettext(user_id_int, "kb_back", app_context=app_context),
        ...
        if len(tag_value) == 2:
            try:
                if tag_value[1] == DELETE_KEY:
                    ...
                    key, value = tag_value
                    if value.startswith("@"):
                        user_repo = app_context.repository_factory.get_user_repository(
                            session
                        )
                ...
                await message.answer(
                    my_gettext(
                        message,
                        "bsn_error",
                        (parse_exception(e, message, app_context),),
                        app_context=app_context,
                    )
                )
        else:
            await message.answer(
                my_gettext(message, "bsn_tag_value", (tag,), app_context=app_context)
            )
...
def parse_exception(
    ...
    if isinstance(exc, EmptyTag):
        return my_gettext(
            user_id_int, "bsn_empty_tag_error", (exc.raw_tag,), app_context=app_context
        )
    if isinstance(exc, LengthError):
        return my_gettext(
            user_id_int,
            "bsn_length_error",
            (len(exc.value), exc.value),
            app_context=app_context,
        )
    return str(exc)
...
async def cmd_gen_data_xdr(bsn_data: BSNData, app_context: AppContext) -> str:
    ...
    return await app_context.stellar_service.build_manage_data_transaction(
        source_account_id=bsn_data.address, data=data
    )


async def bsn_stellar_get_data(
    session: AsyncSession, user_id: int, app_context: AppContext, public_key=None
) -> BSNData:
    if public_key:
        pk = public_key
    else:
...
async def process_tags(
    ...
    await send_message(
        session,
        user_id=message,
        msg=make_tag_message(tags, message.from_user.id, app_context),
        reply_markup=get_bsn_kb(message.from_user.id, app_context, not tags.is_empty()),
        app_context=app_context,
    )


@bsn_router.message(Command("bsn", ignore_case=True))
async def bsn_mode_command(
    message: Message,
    state: FSMContext,
    ...
    **kwargs,
    ...
    tags = await bsn_stellar_get_data(
        session, message.from_user.id, app_context=app_context
    )
    if command.args:
        tag = command.args
        await parse_tag(
...
async def finish_send_bsn(
    ...
    **kwargs,
    ...
    if not await have_free_xlm(
        session=session, user_id=callback_query.from_user.id, app_context=app_context
    ):
        await callback_query.answer(
            my_gettext(callback_query, "low_xlm", app_context=app_context),
            show_alert=True,
        )
        return
...
async def finish_back_bsn(
    ...
    **kwargs,
    ...
    await cmd_show_balance(
        session, callback_query.message.chat.id, state, app_context=app_context
    )
    await callback_query.answer()
