...
async def cmd_start_sign(
    ...
    await cmd_check_xdr(
        session,
        "https://eurmtl.me/sign_tools/" + message.text.split(" ")[1][5:],
        message.from_user.id,
        state,
        app_context=app_context,
    )
...
async def cb_return(
    ...
    if (
        try_sent_xdr
        and datetime.strptime(try_sent_xdr, "%d.%m.%Y %H:%M:%S") > datetime.now()
    ):
...
def get_kb_donate(
    ...
    buttons_list = [["1", "5", "10", "50"], ["100", "300", "1000"]]
...
async def cmd_donate(
    ...
    msg = (
        f"You have {eurmtl_balance} EURMTL\n"
        ...
        if donate_sum > max_sum:
            ...
            if result.success:
                ...
                await state.update_data(
                    ...
                    fsm_after_send=jsonpickle.dumps(cmd_after_donate),
                )
                msg = my_gettext(
                    user_id,
                    "confirm_send",
                    (donate_sum, EURMTL_ASSET.code, father_key, memo),
                    app_context=app_context,
                )
                msg = f"For donate\n{msg}"
                await send_message(
...
async def cb_donate_sum(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
):
    await get_donate_sum(
        session, callback.from_user.id, callback.data, state, app_context=app_context
    )
    await callback.answer()


@router.message(SettingState.send_donate_sum)
async def cmd_donate_sum(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    if message.from_user is None:
        return
    await get_donate_sum(
        session, message.from_user.id, message.text, state, app_context=app_context
    )
...
async def cmd_set_default(
    ...
    try:
        ...
        await update_profile.execute(
            user_id=message.from_user.id, default_address=address
        )
        await session.commit()
    except Exception:
        update_profile = app_context.use_case_factory.create_update_user_profile(
...
async def cq_show_more_less_click(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    await callback.message.edit_text(
        text=await get_start_text(
            session, state, callback.from_user.id, app_context=app_context
        ),
