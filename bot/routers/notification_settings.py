...
async def send_notification_settings_menu(
    ...
    buttons = [
        ...
        [
            types.InlineKeyboardButton(
                text=my_gettext(
                    ...
                    (float2str(user_data.get("min_amount", 0)),),
                    app_context=app_context,
                ),
        ...
        [
            types.InlineKeyboardButton(
                text=my_gettext(
                    user_id,
                    "toggle_wallets_button",
                    (("✅" if is_wallets_notify else "❌"),),
                    app_context=app_context,
                ),
        ...
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "save_button", app_context=app_context),
    ...
    await send_message(
        session, callback, text, reply_markup=keyboard, app_context=app_context
    )
    await callback.answer()


@router.callback_query(F.data == "toggle_token_notify")
async def toggle_token_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    await send_notification_settings_menu(
        callback, state, session, app_context=app_context
    )
...
async def change_amount_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def toggle_wallets_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def save_filter_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    user_id = callback.from_user.id
    public_key = (
        None if user_data.get("for_all_wallets") else user_data.get("public_key")
    )
    ...
    if existing_filter:
        ...
        await send_message(
            session,
            callback,
            my_gettext(user_id, "filter_already_exists", app_context=app_context),
    ...
    await send_message(
        session,
        callback,
        my_gettext(user_id, "filter_saved", app_context=app_context),
        reply_markup=get_kb_return(user_id, app_context=app_context),
        app_context=app_context,
    )
    await callback.answer()
...
async def notification_settings_entry(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    await notification_settings_callback(
        callback,
        NotificationMenuAction(action="list", page=0),
        state,
        session,
        app_context,
    )
...
async def notification_settings_callback(
    ...
    if callback_data.action == "delete_all":
        ...
        await send_message(
            session,
            callback,
            my_gettext(user_id, "all_filters_deleted", app_context=app_context),
            reply_markup=get_kb_return(user_id, app_context=app_context),
            app_context=app_context,
        )
        await callback.answer()
    ...
    for f in page_filters:
        ...
        filter_text = my_gettext(
            user_id,
            "notification_filter_item",
            (op_display, amount_display, asset_display),
            app_context=app_context,
        )
    ...
    if filters_count > 0:
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text=my_gettext(
                        user_id, "kb_delete_all_filters", app_context=app_context
                    ),
...
async def handle_filter_action(
    ...
    if callback_data.action == "delete":
        ...
        if success:
            await callback.answer(
                my_gettext(user_id, "filter_deleted", app_context=app_context)
            )
            # Return to list
            await notification_settings_callback(
                callback,
                NotificationMenuAction(action="list"),
                state,
                session,
                app_context,
            )
            ...
            await callback.answer(
                my_gettext(
                    user_id, "notification_settings_error", app_context=app_context
                ),
                show_alert=True,
            )
        return
    ...
    if callback_data.action == "info":
        ...
        amount_display = (
            float2str(notif_filter.min_amount) if notif_filter.min_amount else "0"
        )
        op_display = notif_filter.operation_type or "Any (*)"
        wallet_display = (
            str(notif_filter.public_key) if notif_filter.public_key else "All Wallets"
        )
        ...
        buttons = [
            [
                types.InlineKeyboardButton(
                    text=my_gettext(
                        user_id, "kb_delete_filter", app_context=app_context
                    ),
                    callback_data=NotificationFilterAction(
                        action="delete", filter_id=filter_id
                    ).pack(),
                )
            ],
            [
...
async def add_filter_menu_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    if not app_context.notification_history:
        await callback.answer(
            my_gettext(user_id, "no_recent_operations", app_context=app_context),
            show_alert=True,
        )
        return
    ...
    for op in recent_ops:
        ...
        buttons.append(
            [
                types.InlineKeyboardButton(
                    text=btn_text, callback_data=f"create_filter_from:{op.id}"
                )
            ]
        )
...
async def create_filter_from_history_callback(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    if not app_context.notification_history:
        await callback.answer(
            my_gettext(user_id, "notification_settings_error", app_context=app_context),
            show_alert=True,
        )
        return
