...
async def cmd_trade_list(
    ...
    if not wallet:
        await send_message(
            session, message, "Кошелек не найден", app_context=app_context
        )
        return

    offers_dicts = await app_context.stellar_service.get_selling_offers(
        wallet.public_key
    )
    ...
    if not offers:
        await send_message(
            session, message, "У вас нет активных ордеров", app_context=app_context
        )
        return
...
async def cmd_trade_cancel(
    ...
    if not result.success:
        await send_message(
            session, message, f"Ошибка: {result.error_message}", app_context=app_context
        )
        return
...
async def cmd_trade_create(
    ...
    try:
        ...
        await send_message(
            session, message, "Не удалось определить сумму", app_context=app_context
        )
        return
    ...
    if sell_code == buy_code:
        await send_message(
            session, message, "Нельзя обменять токен на себя", app_context=app_context
        )
        return
...
async def cmd_market(
    ...
    await send_message(
        ...
        my_gettext(callback, "kb_market", app_context=app_context),
    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_new_order", app_context=app_context),
                callback_data="NewOrder",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_show_order", app_context=app_context),
        ...
        kb_tmp.append(
            [
                types.InlineKeyboardButton(
                    ...
                    callback_data=SaleAssetCallbackData(answer=token.asset_code).pack(),
                )
            ]
        )
...
async def cq_trade_choose_token_sell(
    callback: types.CallbackQuery,
    ...
    for asset in asset_list:
        if asset.asset_code == answer:
            if my_float(asset.balance) == 0.0:
                ...
                for token in asset_list:
                    if (
                        my_float(token.balance) <= 0.0
                        or token.asset_code == asset.asset_code
                    ):
                        continue
                    kb_tmp.append(
                        [
...
async def cq_trade_choose_token_buy(
    callback: types.CallbackQuery,
    callback_data: BuyAssetCallbackData,
    state: FSMContext,
    session: AsyncSession,
    ...
    for asset in asset_list:
        if asset.asset_code == answer:
            ...
            market_link = stellar_get_market_link(
...
async def cb_edit_order(
    ...
    if offer:
        ...
        if o.selling and o.buying and o.selling.asset_code and o.buying.asset_code:
            selling_asset = (
                Asset(o.selling.asset_code, o.selling.asset_issuer)
                if o.selling.asset_issuer
                else Asset.native()
            )
            buying_asset = (
                Asset(o.buying.asset_code, o.buying.asset_issuer)
                if o.buying.asset_issuer
                else Asset.native()
            )
        ...
        await send_message(
            ...
            callback,
            msg,
            reply_markup=get_kb_edit_order(
                callback.from_user.id, app_context=app_context
            ),
            app_context=app_context,
        )
...
def get_kb_edit_order(
    user_id: int, *, app_context: AppContext
) -> types.InlineKeyboardMarkup:
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_edit_sum", app_context=app_context),
                callback_data="EditOrderAmount",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_edit_price", app_context=app_context),
...
async def cmd_edit_order_amount(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    if tmp:
        ...
        try:
            ...
            if use_case_factory:
                ...
                target_obj = next(
                    (b for b in balances if b.asset_code == selling_code), None
                )
...
async def cmd_edit_sale_sum(
    ...
    if send_sum > 0.0:
        ...
        await state.update_data(
            send_sum=send_sum,
            receive_sum=float(receive_sum) * float(send_sum) / float(old_sum),
            msg=None,
        )
...
async def cmd_edit_order_price(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
