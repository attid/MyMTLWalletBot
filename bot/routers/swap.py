...
def build_swap_confirm_message(
    ...
    *,
    ...
    if cancel_offers:
        msg = msg + my_gettext(
            obj, "confirm_cancel_offers", (send_asset,), app_context=app_context
        )
    return msg
...
async def cmd_swap_text(
    ...
    if len(args) < 3:
        await send_message(
            session,
            message,
            "Формат: /swap &lt;сумма&gt; &lt;отдает&gt; &lt;получаем&gt; [проскальзывание%]\nПример: /swap 10 XLM EURMTL 1%",
            app_context=app_context,
        )
        return
    ...
    try:
        ...
        if amount_idx == -1:
            await send_message(
                session,
                message,
                "Не удалось определить сумму (число).",
                app_context=app_context,
            )
            return
        ...
        if amount <= 0:
            await send_message(
                session, message, "Сумма должна быть больше 0.", app_context=app_context
            )
            return
        ...
        if len(remaining_args) < 2:
            await send_message(
                session, message, "Не указаны валюты обмена.", app_context=app_context
            )
            return
        ...
        if len(matching_from) > 1:
            await send_message(
                session,
                message,
                f"Найдено несколько активов с кодом {from_code}. Пожалуйста, используйте меню (кнопку Swap), чтобы выбрать конкретный актив.",
                app_context=app_context,
            )
            return
        ...
        if len(matching_to) == 0:
            ...
            await send_message(
                session,
                message,
                f"Актива {to_code} нет в ваших доверенных линиях (trustlines). Добавьте его через меню.",
                app_context=app_context,
            )
            return
        if len(matching_to) > 1:
            await send_message(
                session,
                message,
                f"Найдено несколько активов с кодом {to_code}. Пожалуйста, используйте меню.",
                app_context=app_context,
            )
            return
        ...
        if not strict_receive:
            ...
            if send_sum > available_balance:
                await send_message(
                    session,
                    message,
                    f"Недостаточно средств. У вас {available_balance} {from_code}",
                    app_context=app_context,
                )
                return
            ...
            if receive_sum <= 0:
                send_asset_obj = (
                    Asset(send_asset_code, send_asset_issuer)
                    if send_asset_issuer
                    else Asset.native()
                )
                receive_asset_obj = (
                    Asset(receive_asset_code, receive_asset_issuer)
                    if receive_asset_issuer
                    else Asset.native()
                )
            ...
            if max_send_sum > available_balance:
                await send_message(
                    session,
                    message,
                    f"Недостаточно средств (с учетом проскальзывания {slippage}% нужно {max_send_sum} {from_code}). У вас {available_balance} {from_code}",
                    app_context=app_context,
                )
                return
        ...
        if not result.success:
            await send_message(
                session,
                message,
                f"Ошибка подготовки обмена: {result.error_message}",
                app_context=app_context,
            )
            return
        ...
        await send_message(
            session, message, "Неверный формат числа.", app_context=app_context
        )
        ...
        await send_message(
            session,
            message,
            "Произошла ошибка при подготовке обмена.",
            app_context=app_context,
        )


@router.callback_query(F.data == "Swap")
async def cmd_swap_01(
    ...
    asset_list = [
        a
        for a in asset_list
        if get_asset_visibility(vis_str, a.asset_code)
        in (ASSET_VISIBLE, ASSET_EXCHANGE_ONLY)
    ]
    ...
    for asset in asset_list:
        if asset.asset_code == answer:
            if my_float(asset.balance) == 0.0:
                ...
                if wallet is None:
                    return
                offers = await app_context.stellar_service.get_selling_offers(
                    wallet.public_key
                )
                blocked_token_sum = 0.0
                for offer in offers:
                ...
                for token in await balance_use_case.execute(
                    user_id=callback.from_user.id
                ):
                    if get_asset_visibility(vis_str, token.asset_code) in (
                        ASSET_VISIBLE,
                        ASSET_EXCHANGE_ONLY,
                    ):
                        if token.asset_code:
                ...
                swap_possible_sum = (
                    "1" if my_float(asset.balance) > 0 else (asset.balance or "0")
                )  # try dont lose path for nfts

                if asset.asset_code:
                    receive_assets = await stellar_check_receive_asset(
                        Asset(asset.asset_code, asset.asset_issuer),
                        swap_possible_sum,
                        asset_list2,
                    )
                    receive_assets = sorted(
                        receive_assets, key=lambda x: str(x)
                    )  # Sort by token name
                    ...
                    for receive_asset in receive_assets:
                        kb_tmp.append(
                            [
                                types.InlineKeyboardButton(
                                    text=f"{receive_asset}",
                                    callback_data=SwapAssetForCallbackData(
                                        answer=str(receive_asset)
                                    ).pack(),
                                )
                            ]
                        )
...
async def cq_swap_choose_token_for(
    callback: types.CallbackQuery,
    callback_data: SwapAssetForCallbackData,
    state: FSMContext,
    session: AsyncSession,
    ...
    for asset in asset_list:
        if asset.asset_code == answer:
            await state.update_data(
                receive_asset_code=asset.asset_code,
...
async def cq_swap_strict_receive(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cq_swap_slippage_click(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cq_swap_cancel_offers_click(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_swap_sum(
    ...
    if send_sum > 0.0:
        ...
        receive_sum_with_slippage = my_round(
            receive_sum * (1 - slippage / 100), 7
        )  # Max 7 decimals for Stellar
...
async def cmd_swap_receive_sum(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
