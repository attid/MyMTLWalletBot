    ...
    buttons = [
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_mtlap_assembly", app_context=app_context),
                callback_data="MTLAPToolsDelegateA",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_mtlap_council", app_context=app_context),
                callback_data="MTLAPToolsDelegateC",
            )
        ],
        [
            types.InlineKeyboardButton(
                text=my_gettext(user_id, "kb_mtlap_recommend", app_context=app_context),
...
async def cmd_mtlap_tools_recommend(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    if not wallet:
        return
    account_details = await app_context.stellar_service.get_account_details(
        wallet.public_key
    )
...
async def cmd_mtlap_send_recommend(
    ...
    if recommendations:
        existing = my_gettext(
            message, "recommend_count", (len(recommendations),), app_context=app_context
        )
    else:
    ...
    if not delegate_exists:
        msg = (
            ...
            + "\n"
            + my_gettext(
                message, "recommend_prompt", (existing,), app_context=app_context
            )
        )
        await send_message(
            session,
    ...
    if not await have_free_xlm(
        session=session, user_id=message.from_user.id, app_context=app_context
    ):
        await send_message(
            session,
            message,
...
async def cmd_mtlap_tools_delegate_a(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_tools_del_delegate_a(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_tools_add_delegate_a(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_send_add_delegate_for_a(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_tools_delegate_c(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_tools_del_delegate_c(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_tools_add_delegate_c(
    callback: types.CallbackQuery,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
...
async def cmd_mtlap_send_add_delegate_for_c(
    message: types.Message,
    state: FSMContext,
    session: AsyncSession,
    app_context: AppContext,
    ...
    # Check if account exists via DI
    account_exists = (
        await app_context.stellar_service.check_account_exists(public_key)
        if not delegate
        else False
    )
