    ...
    async with db_pool.get_session() as session:
        wallet = (
            await session.execute(
                select(MyMtlWalletBot).where(MyMtlWalletBot.user_id == 0)
            )
        ).scalar_one_or_none()
        ...
        if not decrypted_secret:
            logger.error(
                "Failed to decrypt secret! Invalid old password or corrupted data."
            )
            return
        ...
        try:
            ...
            logger.error(
                "Decrypted data is NOT a valid Stellar secret key! Aborting to prevent corruption."
            )
            return
...
async def main():
    ...
    reencrypt_parser = subparsers.add_parser(
        "reencrypt", help="Re-encrypt master key with new password"
    )
    reencrypt_parser.add_argument(
        "--old", required=True, help="Old password used for encryption"
    )
    reencrypt_parser.add_argument(
        "--new", required=True, help="New password to use for encryption"
    )

    # Set secret command
    set_secret_parser = subparsers.add_parser(
        "set_secret",
        help="Manually set master secret (encrypted with current config password)",
    )
    set_secret_parser.add_argument(
        "--secret", required=True, help="The Stellar secret key (starts with S)"
    )
