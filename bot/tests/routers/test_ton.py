...
@pytest.mark.asyncio
async def test_send_ton_full_flow_success(
    mock_telegram, router_app_context, setup_ton_mocks
):
    ...
    await dp.feed_update(
        bot, create_message_update(user_id, valid_address, message_id=2, update_id=2)
    )
    ...
    # 3. Enter Sum
    await dp.feed_update(
        bot, create_message_update(user_id, "1.5", message_id=3, update_id=3)
    )
    ...
    all_texts = " ".join(
        [
            m["data"].get("text", "") + m["data"].get("caption", "")
...
@pytest.mark.asyncio
async def test_send_ton_usdt_full_flow_success(
    mock_telegram, router_app_context, setup_ton_mocks
):
    ...
    # 3. Sum
    await dp.feed_update(
        bot, create_message_update(user_id, "10.0", message_id=3, update_id=3)
    )
    assert (
        await dp.storage.get_state(state_key) == StateSendTonUSDT.sending_confirmation
    )
...
async def test_send_ton_cancel(mock_telegram, router_app_context, setup_ton_mocks):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "ton_no")
    )
...
@pytest.mark.asyncio
async def test_send_ton_invalid_address(
    mock_telegram, router_app_context, setup_ton_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(user_id, "short_address")
    )
