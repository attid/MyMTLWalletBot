...
def setup_fest_mocks(router_app_context):
    ...
    class FestMockHelper:
        ...
        def _setup_defaults(self):
            ...
            self.send_uc.execute = AsyncMock(
                return_value=PaymentResult(success=True, xdr="XDR_FEST")
            )
...
async def test_cmd_fest_menu(mock_telegram, router_app_context, setup_fest_mocks):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(123, "Fest2024")
    )
...
@pytest.mark.asyncio
async def test_cmd_fest_get_sum_flow(
    mock_telegram, router_app_context, setup_fest_mocks
):
    ...
    await dp.storage.update_data(
        state_key, {"level_1": "Participant1", "msg": "Prompt"}
    )
...
@pytest.mark.asyncio
async def test_cmd_reload_fest_menu(
    mock_telegram, router_app_context, setup_fest_mocks
):
    ...
    with patch("routers.fest.load_fest_info", AsyncMock(return_value={"New": "Menu"})):
        await dp.feed_update(
            router_app_context.bot, update, app_context=router_app_context
        )
