...
def get_latest_msg(mock_telegram):
    ...
    msgs = [
        ...
        if r["method"] in ("sendMessage", "editMessageText", "sendPhoto")
    ]
    return msgs[-1] if msgs else None


@pytest.mark.asyncio
async def test_cmd_show_balance_integration(
    mock_telegram, router_app_context, setup_start_mocks
):
...
async def test_get_start_text_stellar(router_app_context, setup_start_mocks):
    ...
    text = await get_start_text(
        MagicMock(), state, user_id, app_context=router_app_context
    )
...
@pytest.mark.asyncio
async def test_cmd_change_wallet_integration(
    mock_telegram, router_app_context, setup_start_mocks
):
...
@pytest.mark.asyncio
async def test_cmd_info_message_with_photo(
    mock_telegram, router_app_context, setup_start_mocks
):
    ...
    assert any(
        r["method"] == "deleteMessage" and int(r["data"]["message_id"]) == 999
        for r in mock_telegram
    )
