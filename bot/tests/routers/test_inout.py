...
async def test_usdt_in_flow(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    mock_app_context.repository_factory.get_user_repository.return_value = (
        mock_user_repo
    )
    ...
    mock_wallet_repo.get_default_wallet = AsyncMock(
        return_value=MagicMock(public_key="GUSER")
    )
    ...
    mock_pay_uc.execute = AsyncMock(
        return_value=PaymentResult(success=True, xdr="FAKE_XDR")
    )
    ...
    mock_app_context.use_case_factory.create_wallet_secret_service.return_value = (
        mock_wallet_secret_service
    )

    # Mock StellarService
    mock_app_context.stellar_service.sign_transaction = AsyncMock(
        return_value="SIGNED_XDR"
    )
    mock_app_context.stellar_service.submit_transaction = AsyncMock(
        return_value={"hash": "tx_hash"}
    )

    with (
        patch("routers.inout.new_wallet_lock", mock_lock),
        patch("routers.inout.get_usdt_balance", AsyncMock(return_value=100.0)),
        patch(
            "routers.inout.check_unconfirmed_usdt_transactions",
            AsyncMock(return_value=False),
        ),
        patch("routers.inout.tron_get_public", return_value="TRON_ADDR"),
        patch(
            "other.stellar_tools.stellar_get_master", new_callable=AsyncMock
        ) as mock_get_master,
    ):
        ...
        sent = [r for r in mock_telegram if r["method"] == "sendMessage"]
        assert any(
            "All works done" in str(m["data"]) or "text" in str(m["data"]) for m in sent
        )
        ...
        mock_app_context.stellar_service.sign_transaction.assert_called_once_with(
            "FAKE_XDR", "S_FAKE_SECRET"
        )
        mock_app_context.stellar_service.submit_transaction.assert_called_once_with(
            "SIGNED_XDR"
        )
...
async def test_usdt_out_flow(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    mock_wallet_repo = MagicMock()
    mock_wallet_repo.get_default_wallet = AsyncMock(
        return_value=MagicMock(use_pin=0, public_key="GMASTER")
    )
    mock_app_context.repository_factory.get_wallet_repository.return_value = (
        mock_wallet_repo
    )
    ...
    mock_pay_uc = MagicMock()
    mock_pay_uc.execute = AsyncMock(
        return_value=PaymentResult(success=True, xdr="XDR_PAY")
    )
    mock_app_context.use_case_factory.create_send_payment.return_value = mock_pay_uc

    with (
        patch(
            "routers.inout.get_usdt_transfer_fee", AsyncMock(return_value=(1.0, 100))
        ),
...
async def test_btc_flow(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    # 3. Enter Sum
    with patch(
        "routers.inout.thoth_create_order", AsyncMock(return_value="ORDER_UUID")
    ):
        await dp.feed_update(
            bot=bot,
            update=types.Update(
...
async def test_cmd_balance_admin(
    ...
    mock_user_repo.get_all_with_usdt_balance = AsyncMock(
        return_value=[("addr1", 100.0, 1), ("addr2", 200.0, 2)]
    )
    mock_app_context.repository_factory.get_user_repository.return_value = (
        mock_user_repo
    )
        ...
        await dp.feed_update(
            ...
            update=types.Update(
                ...
                message=types.Message(
                    ...
                    chat=types.Chat(id=user_id, type="private"),
                    from_user=types.User(
                        id=user_id, is_bot=False, first_name="Admin", username=username
                    ),
                    text="/balance",
                ),
            ),
