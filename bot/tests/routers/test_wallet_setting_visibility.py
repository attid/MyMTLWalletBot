...
def mock_app_context():
    ...
    app_context.localization_service.get_text.side_effect = (
        lambda user_id, key, params=(): str(key)
    )
    return app_context
...
@pytest.mark.asyncio
async def test_asset_visibility_ui_handled_gracefully(
    mock_app_context, mock_wallet_repo
):
    """
        ...
        with patch.object(
            ...
            for c in calls:
                ...
                if "show_alert" in c.kwargs:
                    assert (
                        c.kwargs["show_alert"] is False
                        or c.kwargs["show_alert"] is None
                    )
