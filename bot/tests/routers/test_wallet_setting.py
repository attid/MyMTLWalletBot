...
from routers.wallet_setting import (
    router as wallet_setting_router,
...
async def test_asset_visibility_toggle(
    ...
    cb_data = AssetVisibilityCallbackData(
    ...
    assert (
        ...
        in [r for r in mock_telegram if r["method"] == "answerCallbackQuery"][0][
            "data"
        ]["text"]
    )
...
@pytest.mark.asyncio
async def test_cq_delete_asset_execute(
    mock_telegram,
    mock_horizon,
    ...
    await dp.storage.set_data(
        key=storage_key, data={"assets": jsonpickle.encode(assets)}
    )
    ...
    with patch(
        "routers.wallet_setting.stellar_check_xdr", AsyncMock(side_effect=lambda x: x)
    ):
...
async def test_buy_address_flow(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    mock_repo = MagicMock(spec=IWalletRepository)
    mock_repo.get_default_wallet = AsyncMock(
        side_effect=lambda id: (
            mock_wallet if id != 0 else MagicMock(spec=Wallet, public_key="GMASTER")
        )
    )
    mock_app_context.repository_factory.get_wallet_repository.return_value = mock_repo
    ...
    mock_pay_uc.execute = AsyncMock(
        return_value=PaymentResult(success=True, xdr="XDR_BUY")
    )
    ...
    assert "XDR_BUY" in str(
        await dp.storage.get_data(
...
async def test_address_book_view(
    ...
    mock_app_context.repository_factory.get_addressbook_repository.return_value = (
        mock_repo
    )
