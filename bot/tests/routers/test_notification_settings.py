...
def setup_notification_mocks(router_app_context):
    ...
    class NotificationMockHelper:
        ...
        def _setup_defaults(self):
            ...
            self.ctx.repository_factory.get_notification_repository.return_value = (
                self.notif_repo
            )
...
@pytest.mark.asyncio
async def test_notification_settings_shows_filters(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    setup_notification_mocks.notif_repo.get_by_user_id = AsyncMock(
    ...
    await dp.feed_update(
        ...
        create_callback_update(user_id, NotificationMenuAction(action="list").pack()),
    )
...
@pytest.mark.asyncio
async def test_notification_settings_no_filters(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "NotificationSettings")
    )
...
@pytest.mark.asyncio
async def test_notification_filter_info(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot,
        create_callback_update(
            user_id, NotificationFilterAction(action="info", filter_id=filter_id).pack()
        ),
    )
    ...
    assert (
        NotificationFilterAction(action="delete", filter_id=filter_id).pack() in markup
    )


@pytest.mark.asyncio
async def test_toggle_token_notify(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.storage.update_data(
        state_key,
        {"asset_code": "EURMTL", "cached_asset_code": "EURMTL", "operation_id": 10},
    )

    # 1. Toggle OFF
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "toggle_token_notify")
    )
    ...
    await dp.feed_update(
        router_app_context.bot,
        create_callback_update(user_id, "toggle_token_notify", update_id=2),
    )
    data = await dp.storage.get_data(state_key)
...
@pytest.mark.asyncio
async def test_change_amount_cycle(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "change_amount")
    )
...
@pytest.mark.asyncio
async def test_save_filter_success(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "save_filter")
    )
...
@pytest.mark.asyncio
async def test_delete_all_filters(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot,
        create_callback_update(123, NotificationMenuAction(action="delete_all").pack()),
    )
...
@pytest.mark.asyncio
async def test_delete_single_filter(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot,
        create_callback_update(
            user_id, NotificationFilterAction(action="delete", filter_id=5).pack()
        ),
    )
...
@pytest.mark.asyncio
async def test_add_filter_menu_no_history(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "add_filter_menu")
    )
...
@pytest.mark.asyncio
async def test_add_filter_menu_with_history(
    mock_telegram, router_app_context, setup_notification_mocks
):
...
@pytest.mark.asyncio
async def test_create_filter_from_history(
    mock_telegram, router_app_context, setup_notification_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot,
        create_callback_update(user_id, "create_filter_from:abc123"),
    )
