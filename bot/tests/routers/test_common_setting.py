        ...
        update=types.Update(
            ...
            callback_query=types.CallbackQuery(
                ...
                from_user=types.User(
                    id=user_id, is_bot=False, first_name="T", username="t"
                ),
                chat_instance="ci",
...
async def test_callbacks_lang(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    mock_wallet_repo.get_default_wallet = AsyncMock(
        return_value=MagicMock(
            spec=Wallet, public_key="GKEY", assets_visibility=None, is_free=False
        )
    )
...
async def test_cq_setting_name(mock_telegram, bot, dp, mock_session, mock_app_context):
    ...
    mock_wallet_repo.get_default_wallet = AsyncMock(
        return_value=MagicMock(spec=Wallet, is_free=False, assets_visibility=None)
    )
    mock_app_context.repository_factory.get_wallet_repository.return_value = (
        mock_wallet_repo
    )
