...
@pytest.mark.asyncio
async def test_bsn_mode_command_with_args(
    mock_telegram, router_app_context, setup_bsn_mocks
):
...
@pytest.mark.asyncio
async def test_process_tags_interaction(
    mock_telegram, router_app_context, setup_bsn_mocks
):
...
@pytest.mark.asyncio
async def test_finish_send_bsn(
    mock_telegram, mock_horizon, router_app_context, setup_bsn_mocks
):
...
async def test_finish_back_bsn(mock_telegram, router_app_context, setup_bsn_mocks):
    ...
    with patch("routers.bsn.cmd_show_balance", AsyncMock()) as mock_show_balance:
        update = create_callback_update(
            user_id=user_id, callback_data=BACK_CALLBACK_DATA
        )
        await dp.feed_update(
            bot=router_app_context.bot, update=update, app_context=router_app_context
        )
