...
def setup_mtltools_mocks(router_app_context):
    ...
    class MTLToolsMockHelper:
        ...
        def _setup_defaults(self):
            ...
            self.mock_acc.account.account_id = (
...
async def test_cmd_tools_menu(mock_telegram, router_app_context, setup_mtltools_mocks):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(123, "MTLTools")
    )
    ...
    assert "SealedBoxMenu" not in req["data"]["reply_markup"]


@pytest.mark.asyncio
async def test_cmd_tools_delegate_view(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with patch(
        "routers.mtltools.stellar_get_data",
        AsyncMock(return_value={"mtl_delegate": "GDELEGATE"}),
    ):
        await dp.feed_update(
            router_app_context.bot, create_callback_update(123, "MTLToolsDelegate")
        )
...
@pytest.mark.asyncio
async def test_cmd_tools_del_delegate(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with (
        ...
        patch(
            "routers.mtltools.stellar_get_user_account",
            AsyncMock(return_value=setup_mtltools_mocks.mock_acc),
        ),
        patch("routers.mtltools.cmd_gen_data_xdr", AsyncMock(return_value="XDR_DEL")),
    ):
        await dp.feed_update(
            router_app_context.bot, create_callback_update(123, "MTLToolsDelDelegate")
        )
...
@pytest.mark.asyncio
async def test_add_delegate_flow(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with patch("routers.mtltools.have_free_xlm", AsyncMock(return_value=True)):
        await dp.feed_update(
            ...
            create_callback_update(user_id, "MTLToolsAddDelegate"),
        )
    ...
    with (
        patch(
            "routers.mtltools.stellar_check_account",
        ...
        await dp.feed_update(
            router_app_context.bot,
            create_message_update(user_id, "GTARGET", update_id=2, message_id=2),
        )
...
@pytest.mark.asyncio
async def test_donate_management_flow(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with patch(
        "routers.mtltools.stellar_get_data",
        AsyncMock(return_value={"mtl_donate_test=5": "GADDR"}),
    ):
        await dp.feed_update(
            router_app_context.bot, create_callback_update(user_id, "MTLToolsDonate")
        )
    ...
    with patch("routers.mtltools.have_free_xlm", AsyncMock(return_value=True)):
        await dp.feed_update(
            router_app_context.bot,
            create_callback_update(user_id, "AddDonate", update_id=2),
        )
    ...
    with patch(
        "routers.mtltools.stellar_check_account", AsyncMock(return_value=mock_target)
    ):
        await dp.feed_update(
            router_app_context.bot,
            create_message_update(user_id, "GTARGET", update_id=3, message_id=3),
        )
    assert await dp.storage.get_state(state_key) == StateTools.donate_name

    await dp.feed_update(
        router_app_context.bot,
        create_message_update(user_id, "Charity", update_id=4, message_id=4),
    )
    ...
    with (
        ...
        patch(
            "routers.mtltools.cmd_gen_data_xdr", AsyncMock(return_value="XDR_DONATE")
        ),
    ):
        await dp.feed_update(
            router_app_context.bot,
            create_message_update(user_id, "10.5", update_id=5, message_id=5),
        )
...
@pytest.mark.asyncio
async def test_bim_management_flow(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with patch(
        "routers.mtltools.stellar_get_data",
        AsyncMock(return_value={"bod_shop": "GADDR"}),
    ):
        await dp.feed_update(
            router_app_context.bot, create_callback_update(user_id, "MTLToolsAddBIM")
        )
    ...
    with patch("routers.mtltools.have_free_xlm", AsyncMock(return_value=True)):
        await dp.feed_update(
            router_app_context.bot,
            create_callback_update(user_id, "AddBIM", update_id=2),
        )
    ...
    with (
        ...
        await dp.feed_update(
            router_app_context.bot,
            create_message_update(user_id, "ShopName", update_id=4, message_id=4),
        )
...
@pytest.mark.asyncio
async def test_cmd_tools_update_multi(
    mock_telegram, router_app_context, setup_mtltools_mocks
):
    ...
    with (
        ...
        patch(
            "routers.mtltools.check_account_id_from_grist", AsyncMock(return_value=True)
        ),
        patch(
            "routers.mtltools.stellar_get_multi_sign_xdr",
        ...
        await dp.feed_update(
            router_app_context.bot,
            create_callback_update(user_id, "MTLToolsUpdateMulti"),
        )
