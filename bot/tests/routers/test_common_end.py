...
def setup_common_end_mocks(router_app_context, horizon_server_config):
    ...
    class CommonEndMockHelper:
        ...
        def _setup_defaults(self):
            ...
            self.p_check_acc = patch(
                "other.stellar_tools.stellar_check_account",
                return_value=MagicMock(account_id="GVALID", memo=None),
            )
            self.p_check_xdr = patch(
                "other.stellar_tools.stellar_check_xdr", return_value="VALID_XDR"
            )
            ...
            self.user_repo.get_account_by_username = AsyncMock(
                return_value=("GFORWARDED", 456)
            )
            ...
            self.wallet_repo.get_default_wallet = AsyncMock(
                return_value=MagicMock(use_pin=0)
            )
    ...
    return types.Update(
        ...
        message=types.Message(
            ...
            from_user=types.User(
                id=user_id, is_bot=False, first_name="Test", username="test"
            ),
            text=text,
            **kwargs,
        ),
...
async def test_cmd_last_route_xdr_base64(
    ...
    xdr = base64.b64encode(
        b"This is a long enough string to be considered XDR and bypass length checks"
    ).decode()

    with patch("routers.common_end.cmd_check_xdr", AsyncMock()) as mock_check:
        await dp.feed_update(
            router_app_context.bot, create_custom_message_update(user_id, xdr)
        )
...
async def test_cmd_last_route_sign_link(
    ...
    entity = types.MessageEntity(
        type="url", offset=13, length=32, url="https://eurmtl.me/sign_tools/abcde"
    )
...
@pytest.mark.asyncio
async def test_cmd_last_route_forwarded_from_user(
    mock_telegram, router_app_context, setup_common_end_mocks
):
    ...
    user_id = 123
    fwd_user = types.User(
        id=456, is_bot=False, first_name="Forwarded", username="itolstov"
    )
    update = create_custom_message_update(user_id, "Hello", forward_from=fwd_user)

    await dp.feed_update(router_app_context.bot, update)

    setup_common_end_mocks.user_repo.get_account_by_username.assert_called_with(
        "@itolstov"
    )
    req = get_latest_msg(mock_telegram)
    assert "choose_token" in req["data"]["text"]
...
@pytest.mark.asyncio
async def test_cmd_last_route_delete_normal(
    mock_telegram, router_app_context, setup_common_end_mocks
):
    ...
    user_id = 123
    await dp.feed_update(
        router_app_context.bot, create_custom_message_update(user_id, "Just some text")
    )
