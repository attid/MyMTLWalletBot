...
def setup_trade_mocks(router_app_context):
    ...
    class TradeMockHelper:
        ...
        def _setup_defaults(self):
            ...
            manage_uc = MagicMock()
            manage_uc.execute = AsyncMock(
                return_value=PaymentResult(success=True, xdr="XDR_MANAGE_OFFER")
            )
...
@pytest.mark.asyncio
async def test_cmd_sale_new_order_start(
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(123, "NewOrder")
    )
...
@pytest.mark.asyncio
async def test_cq_trade_choose_token_sell(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.storage.set_data(
        storage_key, {"assets": jsonpickle.encode(setup_trade_mocks.balances)}
    )

    cb_data = SaleAssetCallbackData(answer="XLM").pack()
    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, cb_data)
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_creation_flow(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    # Enter Sell Amount
    await dp.feed_update(
        router_app_context.bot, create_message_update(user_id, "100.0")
    )
    ...
    await dp.feed_update(
        ...
        create_message_update(user_id, "10.0", message_id=2, update_id=2),
    )
    ...
    args = router_app_context.use_case_factory.create_manage_offer.return_value.execute.call_args[
        1
    ]
...
@pytest.mark.asyncio
async def test_cmd_show_orders(
    mock_telegram, mock_horizon, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_callback_update(123, "ShowOrders")
    )
...
@pytest.mark.asyncio
async def test_delete_order_execution(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.storage.set_data(
        storage_key, {"offers": jsonpickle.encode([offer]), "edit_offer_id": 555}
    )

    await dp.feed_update(
        router_app_context.bot, create_callback_update(user_id, "DeleteOrder")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_list_empty(
    mock_telegram, mock_horizon, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade list")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_list_with_orders(
    mock_telegram, mock_horizon, router_app_context, setup_trade_mocks
):
...
@pytest.mark.asyncio
async def test_cmd_trade_cancel_not_found(
    mock_telegram, mock_horizon, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade cancel 99999")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_cancel_success(
    mock_telegram, mock_horizon, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade cancel 12345")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_create_with_price(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade 10 XLM USDT 0.25")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_create_with_receive_amount(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade 10 XLM 2.5 USDT")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_insufficient_balance(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade 200 XLM EURMTL 0.5")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_no_trustline(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade 10 XLM USDC 0.5")
    )
...
@pytest.mark.asyncio
async def test_cmd_trade_same_asset(
    mock_telegram, router_app_context, setup_trade_mocks
):
    ...
    await dp.feed_update(
        router_app_context.bot, create_message_update(123, "/trade 10 XLM XLM 1")
    )
