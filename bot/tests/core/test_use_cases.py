...
async def test_get_wallet_balance_success(mock_horizon, horizon_server_config):
    ...
    wallet = Wallet(
        id=1, user_id=user_id, public_key=public_key, is_default=True, is_free=False
    )
    ...
    mock_horizon.set_account(
        public_key, balances=[{"asset_type": "native", "balance": "100.0000000"}]
    )
...
async def test_get_wallet_balance_with_address(mock_horizon, horizon_server_config):
    ...
    mock_horizon.set_account(
        public_key, balances=[{"asset_type": "native", "balance": "50.0000000"}]
    )
    mock_horizon.set_offers(public_key, [])
...
async def test_register_new_user():
    ...
    mock_wallet_repo.create.return_value = Wallet(
        id=5, user_id=10, public_key="PUB", is_default=True, is_free=True
    )
...
async def test_register_existing_user():
    ...
    existing_wallet = Wallet(
        id=6, user_id=20, public_key="OLD_KEY", is_default=True, is_free=True
    )
