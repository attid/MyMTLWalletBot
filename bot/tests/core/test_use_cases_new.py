...
async def test_add_wallet_success(mock_wallet_repo):
    ...
    mock_wallet_repo.create.return_value = Wallet(
        id=1, user_id=123, public_key="PUB", is_default=True, is_free=True
    )

    result = await use_case.execute(
        user_id=123, public_key="PUB", secret_key="SEC", is_free=True, is_default=True
    )
...
async def test_add_wallet_limit_reached(mock_wallet_repo):
    use_case = AddWallet(mock_wallet_repo)
    mock_wallet_repo.count_free_wallets.return_value = (
        3  # Max limit is >2 (so 0,1,2 ok, 3 fails)
    )

    with pytest.raises(ValueError, match="Maximum number of free wallets reached"):
        await use_case.execute(
            user_id=123, public_key="PUB", secret_key="SEC", is_free=True
        )
...
async def test_add_wallet_read_only(mock_wallet_repo):
    use_case = AddWallet(mock_wallet_repo)
    mock_wallet_repo.create.return_value = Wallet(
        id=1, user_id=123, public_key="PUB", is_default=False, is_free=False, use_pin=10
    )

    # is_read_only=True
    await use_case.execute(
        user_id=123, public_key="PUB", secret_key=None, is_read_only=True
    )
...
async def test_update_user_profile(mock_user_repo):
    ...
    existing_user = User(
        id=123, username="old", language="en", default_address="addr", can_5000=0
    )
