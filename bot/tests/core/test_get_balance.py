...
async def test_get_balance_includes_issued_assets():
    ...
    # Mock Issued Assets
    mock_stellar.get_assets_by_issuer = AsyncMock(
        return_value=[{"asset_code": "MYTOKEN", "asset_type": "credit_alphanum12"}]
    )
...
@pytest.mark.asyncio
async def test_get_balance_refreshes_when_cache_is_older_than_one_hour():
    mock_repo = MagicMock()
    mock_stellar = MagicMock()
    ...
    wallet = Wallet(
        ...
        last_event_id="10",
        balances_updated_at=datetime.now(UTC) - timedelta(hours=2),
    )
    mock_repo.get_default_wallet = AsyncMock(return_value=wallet)
    mock_repo.update = AsyncMock()
