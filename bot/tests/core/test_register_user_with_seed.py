...
async def test_register_new_user_with_seed():
    ...
    mock_user_repo.create.return_value = User(
        id=user_id, username=username, language=language
    )
    ...
    user, wallet = await use_case.execute(
        user_id, username, language, pub_key, sec_key, seed_key
    )
...
async def test_register_existing_user_missing_wallet_recreated_with_seed():
    ...
    mock_user_repo.get_by_id.return_value = User(
        id=user_id, username=username, language="en"
    )
