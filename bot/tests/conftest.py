...
def get_free_port(start_port=8000, end_port=9000, retries=10):
    ...
    for _ in range(retries):
        ...
        try:
            ...
            continue
    raise RuntimeError(
        f"Could not find a free port in range {start_port}-{end_port} after {retries} attempts"
    )
...
async def mock_telegram(telegram_server_config):
        ...
        return web.json_response(
            {
                ...
                "result": {
                    ...
                    "date": 1234567890,
                    "chat": {
                        "id": int(request_data.get("chat_id", 123)),
                        "type": "private",
                    },
                    "text": "forwarded",
                },
    ...
    async def send_poll(request):
        ...
        data = dict(data)
        received_requests.append(
            {"method": "sendPoll", "token": request.match_info["token"], "data": data}
        )
    ...
    await runner.setup()
    site = web.TCPSite(
        runner, telegram_server_config["host"], telegram_server_config["port"]
    )
    await site.start()
...
@pytest.fixture
async def router_app_context(
    mock_app_context, router_bot, horizon_server_config, mock_horizon
):
    """
    ...
    mock_app_context.stellar_service = StellarService(
...
async def mock_horizon(horizon_server_config):
    ...
    class HorizonMockState:
        ...
        def set_account(
            ...
            self.accounts[account_id] = {
                ...
                "signers": signers
                or [{"key": account_id, "weight": 1, "type": "ed25519_public_key"}],
                "thresholds": {
                    "low_threshold": 0,
                    "med_threshold": 0,
                    "high_threshold": 0,
        ...
        def set_transaction_response(
            ...
            if error:
                self.transaction_response["extras"] = {
    ...
    async def get_account_offers(request):
        ...
        return web.json_response({"_embedded": {"records": offers}})

    @routes.get("/offers")
    async def get_offers(request):
        """General offers endpoint with query params."""
        params = dict(request.query)
        state.requests.append({"endpoint": "offers", "method": "GET", "params": params})
    ...
    async def get_strict_send_paths(request):
        ...
        params = dict(request.query)
        state.requests.append(
            {"endpoint": "paths/strict-send", "method": "GET", "params": params}
        )
        return web.json_response({"_embedded": {"records": state.paths}})
    ...
    async def get_operations(request):
        ...
        params = dict(request.query)
        state.requests.append(
            {"endpoint": "operations", "method": "GET", "params": params}
        )
        ...
        return web.json_response({"_embedded": {"records": state.operations}})
    ...
    async def get_strict_receive_paths(request):
        ...
        params = dict(request.query)
        state.requests.append(
            {"endpoint": "paths/strict-receive", "method": "GET", "params": params}
        )
    ...
    async def submit_transaction(request):
        ...
        state.requests.append(
            {"endpoint": "transactions", "method": "POST", "data": dict(data)}
        )
    ...
    async def get_transactions(request):
        ...
        params = dict(request.query)
        state.requests.append(
            {"endpoint": "transactions", "method": "GET", "params": params}
        )
    ...
    async def get_payments(request):
        ...
        params = dict(request.query)
        state.requests.append(
            {"endpoint": "payments", "method": "GET", "params": params}
        )
                    ...
                    "records": [
                        {
                            ...
                            "destination_asset_type": params.get(
                                "destination_asset_type", "credit_alphanum12"
                            ),
                            "destination_asset_code": params.get(
                                "destination_asset_code"
                            ),
                            "destination_asset_issuer": params.get(
                                "destination_asset_issuer"
                            ),
                            "destination_amount": params.get(
    ...
    async def catch_all_get(request):
        ...
        return web.json_response(
            {"error": f"Mock endpoint not implemented: GET /{path}"}, status=404
        )
    ...
    async def catch_all_post(request):
        ...
        state.requests.append({"endpoint": f"UNHANDLED:{path}", "method": "POST"})
        return web.json_response(
            {"error": f"Mock endpoint not implemented: POST /{path}"}, status=404
        )
