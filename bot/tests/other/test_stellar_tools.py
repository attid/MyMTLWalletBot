...
async def test_stellar_delete_account_uses_master_source_address(mock_keypairs):
    ...
    with (
        patch("other.stellar_tools.ServerAsync") as MockServerAsync,
        ...
        server_instance = (
            MagicMock()
        )  # Use MagicMock for synchronous methods like accounts()
        ...
        delete_acc_details = {
            "balances": [
                {"asset_type": "native", "balance": "10.0", "asset_code": "XLM"}
            ],
            "data": {},  # No data entries
        }
        # Mock master account details (for trustlines check)
        master_acc_details = {"balances": []}
        ...
        await stellar_delete_account(
            master_kp, delete_kp, master_source_address=str(master_source_address)
        )
...
async def test_stellar_delete_all_deleted_passes_correct_address():
    ...
    with (
        ...
        patch(
            "other.stellar_tools.stellar_delete_account", new_callable=AsyncMock
