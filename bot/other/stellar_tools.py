...
from other.asset_visibility_tools import (
    ...
    ASSET_VISIBLE,
)  # Import for asset visibility tools
...
def stellar_get_transaction_builder(xdr: str) -> TransactionBuilder:
    ...
    if (
        existing_transaction.preconditions
        and existing_transaction.preconditions.time_bounds
    ):  # type: ignore[union-attr,index]
        transaction_builder.set_timeout(
            existing_transaction.preconditions.time_bounds.max_time
        )  # type: ignore[union-attr,index]
    else:
...
async def stellar_add_trust(
    user_key: str, asset: Asset, xdr: Optional[str] = None, delete: bool = False
):  # type: ignore[misc]
    if xdr:
...
async def stellar_close_asset(
    user_key: str, asset: Asset, amount, xdr: Optional[str] = None
):
    if xdr:
        transaction = stellar_get_transaction_builder(xdr)
    else:
...
async def stellar_user_sign(
    session: AsyncSession, xdr: str, user_id: int, user_password: str
):
    user_key_pair = await stellar_get_user_keypair(session, user_id, user_password)
    return stellar_sign(xdr, user_key_pair.secret)


async def stellar_user_sign_message(
    session: AsyncSession, msg: str, user_id: int, user_password: str
) -> str:
    user_key_pair = await stellar_get_user_keypair(session, user_id, user_password)
...
async def stellar_get_selling_offers_sum(
    session: AsyncSession, user_id: int, sell_asset_filter: Asset
):
    """
...
async def stellar_del_selling_offers(
    transaction: TransactionBuilder, account_id: str, sell_asset_filter: Asset
):
    """
...
async def stellar_get_user_keypair(
    session: AsyncSession, user_id: int, user_password: str
) -> Keypair:
...
async def stellar_get_user_account(
    session: AsyncSession, user_id: int, public_key=None
) -> Account:
    if public_key:
        result = public_key
    else:
...
async def stellar_get_master(session: AsyncSession) -> Keypair:
    return await stellar_get_user_keypair(
        session, 0, config.master_password.get_secret_value()
    )


async def stellar_delete_account(
    master_account: Keypair, delete_account: Keypair, master_source_address: str
):
    async with ServerAsync(
        horizon_url=config.horizon_url, client=AiohttpClient()
    ) as server:
        ...
        master_account_details = (
            await server.accounts().account_id(master_source_address).call()
        )
        master_account_trustlines = {
            (balance["asset_code"], balance["asset_issuer"]): balance
            for balance in master_account_details["balances"]
        ...
        for balance in account["balances"]:
            if balance["asset_type"] != "native":
                ...
                if float(balance["balance"]) > 0.0:
                    ...
                    ) not in master_account_trustlines:
                        # Trustline source must be the account HOLDING the assets (Master)
                        transaction.append_change_trust_op(
                            asset=asset, source=master_source_address
                        )

                    # Payment destination is Master
                    transaction.append_payment_op(
                        destination=master_source_address,
                        amount=balance["balance"],
                        asset=asset,
                        source=delete_account.public_key,
                    )
                transaction.append_change_trust_op(
                    asset=asset, limit="0", source=delete_account.public_key
                )

        data_entries = account.get("data", {})
        for data_key in data_entries.keys():
            transaction.append_manage_data_op(
                data_name=data_key, data_value=None, source=delete_account.public_key
            )

        transaction.append_account_merge_op(
            master_source_address, delete_account.public_key
        )
        ...
        if delete_account.signing_key:
            await async_stellar_send(
                stellar_sign(
                    stellar_sign(xdr, master_account.secret), delete_account.secret
                )
            )
        else:
            return xdr
...
async def stellar_get_balance_str(
    session: AsyncSession, user_id: int, public_key=None, state=None
) -> str:
    ...
    if (
        public_key is None
        and state
        and not (await state.get_data()).get("show_more", False)
    ):
    ...
    for balance in balances:
        ...
        if balance.selling_liabilities and float(balance.selling_liabilities) > 0:
            ...
            free = float2str(
                float(balance.balance) - float(balance.selling_liabilities), short=True
            )
            result += f"{balance.asset_code} : {free} (+{lock}={full})\n"
        else:
            result += (
                f"{balance.asset_code} : {float2str(balance.balance, short=True)}\n"
            )
...
async def stellar_get_balances(
    session,
    user_id: int,
    public_key=None,
    ...
    try:
        ...
        if asset_filter and result:
            result = [
                balance for balance in result if balance.asset_code == asset_filter
            ]
...
async def stellar_get_data(
    ...
    async with ServerAsync(
        ...
        data = MyAccount.from_dict(
            await server.accounts().account_id(user_account.account.account_id).call()
        ).data
...
async def stellar_get_offers(
    ...
    async with ServerAsync(
        ...
        offers = MyOffers.from_dict(
            await server.offers()
            .for_seller(user_account.account.account_id)
            .limit(90)
            .call()
        )
...
async def stellar_has_asset_offers(
    session: AsyncSession, user_id: int, asset: Asset
) -> bool:
    offers = await stellar_get_offers(session, user_id)
    for offer in offers:
        if (
            offer.selling
            and offer.selling.asset_code == asset.code
            and offer.selling.asset_issuer == asset.issuer
        ):
            return True
        if (
            offer.buying
            and offer.buying.asset_code == asset.code
            and offer.buying.asset_issuer == asset.issuer
        ):
            return True
    return False


async def stellar_change_password(
    ...
    try:
        ...
        logger.info(
            [
                ...
                send_asset.code + " " + send_sum + " " + receive_asset.code,
                ex,
            ]
        )
        return "0"
...
# TODO: Remove after routers/swap.py is refactored
async def stellar_check_receive_asset(
    send_asset: Asset, send_sum: str, receive_assets: List[Asset]
) -> List[str]:
    """
    ...
    try:
        async with ServerAsync(
            ...
            while receive_assets:
                ...
                call_result = await server.strict_send_paths(
                    send_asset, send_sum, current_batch
                ).call()
...
async def stellar_get_multi_sign_xdr(public_key) -> str:
    async with ServerAsync(
        ...
        public_issuer_signers = MyAccount.from_dict(
            await server.accounts().account_id(public_issuer).call()
        ).signers
        public_issuer_signers = sorted(
            public_issuer_signers, key=lambda x: (x.weight, x.key), reverse=True
        )
        public_issuer_signers = [
            signer for signer in public_issuer_signers if signer.key != public_issuer
        ]

        account = MyAccount.from_dict(
            await server.accounts().account_id(public_key).call()
        )
        data = account.data
    ...
    async with ServerAsync(
        ...
        if account.thresholds.med_threshold != 15:
            transaction.append_set_options_op(
                med_threshold=15, low_threshold=15, high_threshold=15
            )
