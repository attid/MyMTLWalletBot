...
async def main() -> None:
    ...
    print(
        f"User-friendly (bounceable): {account.address.to_userfriendly(is_bounceable=True)}"
    )
    ...
    usdt_balance = await tonapi.accounts.get_jetton_balance(
        account_id=wallet.address.to_str(), jetton_id=USDT_MASTER_ADDRESS.to_str()
    )
...
async def main0():
    ...
    print(result)  # [242]
    wallet = await WalletV4R2.from_address(
        provider=client, address="EQBvW8Z5huBkMJYdnfAEM5JqTNkuWX3diqYENkWsIL0XggGG"
    )
    ...
    result = await client.run_get_method(
        ...
        method="get_router_data",
        stack=[],
    )
    print(
        result
    )  # [0, <Slice 267[80093377825F7267A94C4EF8966051F874BF125171483071FC33E1E05EBFF4DF6E00] -> 0 refs>, <Cell 130[0000000000000000000000000000000000] -> 1 refs>, <Cell 80[FF00F4A413F4BCF2C80B] -> 1 refs>, <Cell 80[FF00F4A413F4BCF2C80B] -> 1 refs>, <Cell 80[FF00F4A413F4BCF2C80B] -> 1 refs>]
    ...
    print(
        result
    )  # [<Slice 267[801B54D587424F634D8AC9DC74071390A2EBDA9F0410E4F19F734DD133C9F136F4A0] -> 0 refs>]
    jetton_wallet_address = result[0].load_address()
    print(jetton_wallet_address)  # EQDapqw6EnsabFZO46A4nIUXXtT4IIcnjPuabomeT4m3paST

    result = await client.run_get_method(
        address="EQDapqw6EnsabFZO46A4nIUXXtT4IIcnjPuabomeT4m3paST",
        method="get_wallet_data",
        stack=[],
    )
    print(
        result
    )  # [2005472, <Slice 267[800DEB78CF30DC0C8612C3B3BE0086724D499B25CB2FBBB154C086C8B58417A2F040] -> 0 refs>, <Slice 267[800E538276DBE580F97E140D56C7A695E8A9E7A641D8379B1F6B8DA49A100D42F840] -> 0 refs>, <Cell 80[FF00F4A413F4BCF2C80B] -> 1 refs>]
