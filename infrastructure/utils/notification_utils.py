...
def decode_db_effect(operation: TOperations, decode_for: str, user_id: int, app_context: AppContext = None, localization_service: Any = None):
    ...
    if operation.operation == 'trade':
        return my_gettext(user_id, 'info_trade',
                          (account_link, float2str(operation.amount1), str(operation.code1),
                           float2str(operation.amount2), str(operation.code2), op_link), 
